...
### Added
- **A real, public self-service registration flow.** There was previously no way for a visitor to create an account at all -- every "Enroll now" button and the login page itself led only to a credentials form with no path to create a new one; the only account-creation path in the whole app was a temporary, secret-gated admin test route. `/register` is now the genuine, public path: it creates a real Prisma-backed `User` + `ParentProfile` (+ `PARENT` role grant), the same records the admin test route created, and is reachable from the homepage pricing section and the login page.
- **Required email verification before an account can sign in.** New accounts are created with `UserStatus.PENDING_VERIFICATION` (an enum value that already existed in the schema but was never actually used anywhere) instead of being signed in immediately. A confirmation link is emailed (via the existing notification dispatcher); clicking it and submitting the confirmation form flips the account to `ACTIVE` and signs the visitor in. Login already rejected any non-`ACTIVE` account with the same generic "incorrect email or password" message used for a wrong password (the established anti-enumeration pattern in this codebase), so an unverified account genuinely cannot be used until confirmed. Confirmation only happens on an explicit form submit, never on page load -- the same defense `reset-password` already uses against corporate email-security scanners that pre-fetch every link in an inbound email, which would otherwise silently burn the token before the real person clicks it.
  - New `EmailVerificationToken` model (`prisma/schema.prisma`), same shape and convention as the existing `PasswordResetToken`: only a sha256 hash of the token is ever persisted, single-use via `usedAt`, time-boxed via `expiresAt` (24 hours, longer than password-reset's 1 hour, since a verification link is often opened well after signup rather than within minutes).
  - New `src/lib/auth/emailVerification.ts`, new `/verify-email` page, new `EMAIL_VERIFICATION` notification event type, new `VERIFY_EMAIL_SUBMIT_PER_IP` rate limit.
- **A public "1-Day Free Trial" plan, visible to every visitor on the homepage pricing section** (not gated behind picking a paid plan first) -- a standout banner above the 3 existing plan cards linking straight into the new registration flow with `?plan=plan-starter&trial=1`.
  - **Card required upfront** (the founder's explicit choice over a zero-friction, no-card signup): trial checkout runs through the same real Stripe Checkout flow as a normal subscription, using Stripe's native `trial_period_days: 1` so the card is authorized but not charged for the first day, then auto-converts to the Starter plan ($49/mo, the cheapest tier) unless the visitor cancels first from the billing page's existing self-service Stripe portal. No new payment infrastructure was built -- this reuses `createStripeCheckoutSession` end to end, now accepting an optional `trialDays` parameter.
  - New `SubscriptionStatus.TRIALING` is now actually produced by a real code path for the first time (the enum value already existed but, like `PENDING_VERIFICATION`, was previously unused). Trial expiry is enforced the same "lazy expiry" way as `PasswordResetToken`/`EmailVerificationToken` -- by comparing the stored `currentPeriodEnd` against the current time at the point of use -- rather than a background job, since this codebase has none. New `src/lib/auth/subscriptionAccess.ts` (`getParentAccessLevel`) is the single choke point every gate below reads from.
- **Feature scoping for the trial** (my call, as the founder explicitly delegated which features to restrict): the trial keeps full access to every self-serve, audio-driven, zero-marginal-cost feature -- Pronunciation Studio, Quran Studio recitation, Phonics Arcade, Story Reader, Printables Hub -- specifically because these are what best demonstrate how strong the platform is at teaching Arabic, per the founder's stated goal. Two things are hard-gated, both server-side (not just hidden in the UI) so a direct form submission can't bypass them:
  - **Live class enrollment** (`/parent/enroll`) -- the one feature that consumes real teacher capacity rather than being a self-serve recorded/audio experience, and the clearest, highest-value reason to convert to a paid plan. A trial account sees an upsell card in place of the class-enrollment grid; `handleEnroll`'s server action independently re-checks the same access level before enrolling.
  - **A 2nd child profile** (`/parent/children`) -- a trial is scoped to exploring the platform with one child; adding a second is treated as the clear signal of a household ready to convert. `handleAddChild`'s server action re-checks the current child count server-side the same way.
  - Not exhaustively gated in this pass, deliberately: messaging, the AI Learning Plan, CEFR guidance, and certificates have no single, easily-located choke-point file the way enrollment and child-creation do, so they were left out of scope rather than half-covered. Worth a dedicated follow-up if the trial is found to be over-generous in practice.
- **Parent dashboard trial countdown**: the subscription status card already showed a "Trialing" pill; it now also shows hours remaining (computed the same lazy-expiry way) and an "Upgrade now" link straight to checkout while trialing, replacing the normal renewal-date line.
- All new UI strings (trial pricing banner, trial registration copy, email-verification pages, dashboard countdown) added across all 6 locale dictionaries (ar, en, es, it, nl, tr), matching the translation-parity convention used throughout this engagement.

### Fixed
- **`fulfillCheckoutSession` always hardcoded a completed checkout's subscription as `ACTIVE`, regardless of what Stripe actually reported.** This was a real, pre-existing bug (found while wiring the trial through this exact function) that would have completely defeated the trial the moment it shipped: a `trial_period_days` checkout would have been recorded as a fully active, unrestricted paid subscription from the instant it completed, since nothing ever produced `TRIALING`. Now reads the real Stripe subscription status (already being fetched for the period-end date) and maps it via the existing `mapStripeSubscriptionStatus()` helper before writing it.

### Validated
- `next lint` passes with zero new warnings (the one pre-existing warning in `ClassroomLive.tsx` is unrelated and untouched by this change).
- `tsc --noEmit` cannot be run to a clean pass in this sandbox at all right now -- not specific to this change. The sandbox's network policy blocks `binaries.prisma.sh`, so `npx prisma generate` cannot download a real query engine, and the `@prisma/client` package currently sitting in `node_modules` is the pre-generate placeholder stub (`PrismaClient = any`, no model or enum exports at all). Every file in the project that imports an enum or model type from `@prisma/client` -- hundreds of pre-existing, untouched files included -- fails the same way (`TS2305: has no exported member 'X'`) purely because of this stub, not because of any code defect; this is the same known limitation noted in the QR/Printables entry above. I manually re-read every new and edited file in this release end to end in place of a clean local typecheck. Vercel's own build installs dependencies fresh (its `postinstall` script runs `prisma generate` with full network access), so the Deployment Check's `Typecheck` step is the real gate for this release, same as every prior release in this engagement -- confirming it passes on the live deployment is the next step before this is marked fully validated.
- Not yet live-tested end-to-end on production: registering through the new trial CTA, confirming the verification email dispatches and its link works, completing Stripe test-mode checkout, confirming the resulting `Subscription` records as `TRIALING` (not `ACTIVE`), and confirming class enrollment is actually blocked with the upsell message. Planned as the next step after this deploys.
