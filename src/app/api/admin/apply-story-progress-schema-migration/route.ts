...
// Same reasoning as apply-billing-schema-migration / apply-gradebook-schema-migration:
// force this to run only when actually requested, not during the Vercel
// build's static-render pass.
...
// the new StoryProgress model to prisma/schema.prisma therefore does NOT, by
...
export async function GET(request: NextRequest) {
  ...
  const statements = [
    `CREATE TABLE IF NOT EXISTS "story_progress" (
      ...
      "quizScorePercentage" INTEGER NOT NULL DEFAULT 0,
    ...
    `DO $$ BEGIN
      ALTER TABLE "story_progress"
        ADD CONSTRAINT "story_progress_studentId_fkey"
        FOREIGN KEY ("studentId") REFERENCES "student_profiles"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    EXCEPTION WHEN duplicate_object THEN NULL; END $$;`,
    `CREATE UNIQUE INDEX IF NOT EXISTS "story_progress_studentId_storyId_key" ON "story_progress"("studentId", "storyId");`,
  ];
  ...
  return NextResponse.json({
    message: "Story progress schema migration applied successfully (safe to re-run).",
    appliedStatements: applied,
    reminder:
      "Delete this route (src/app/api/admin/apply-story-progress-schema-migration) or remove SEED_ADMIN_SECRET from Vercel once you're done.",
  });
}
