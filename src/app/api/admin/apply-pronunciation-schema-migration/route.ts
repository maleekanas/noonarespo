...
// the new PronunciationAttempt model to prisma/schema.prisma therefore does
...
export async function GET(request: NextRequest) {
  ...
  const statements = [
    `CREATE TABLE IF NOT EXISTS "pronunciation_attempts" (
      ...
      "pitchAccuracy" INTEGER NOT NULL,
    ...
    `CREATE INDEX IF NOT EXISTS "pronunciation_attempts_studentId_idx" ON "pronunciation_attempts"("studentId");`,
  ];
  ...
  return NextResponse.json({
    message: "Pronunciation schema migration applied successfully (safe to re-run).",
    appliedStatements: applied,
    reminder:
      "Delete this route (src/app/api/admin/apply-pronunciation-schema-migration) or remove SEED_ADMIN_SECRET from Vercel once you're done.",
  });
}
