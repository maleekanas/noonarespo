...
// the new PlacementAttempt model to prisma/schema.prisma therefore does NOT,
...
export async function GET(request: NextRequest) {
  ...
  const statements = [
    `CREATE TABLE IF NOT EXISTS "placement_attempts" (
      ...
      "recommendedLevelCode" TEXT NOT NULL,
      "recommendedLevelTitleAr" TEXT NOT NULL,
      "recommendedLevelTitleEn" TEXT NOT NULL,
    ...
    `DO $$ BEGIN
      ALTER TABLE "placement_attempts"
        ADD CONSTRAINT "placement_attempts_studentId_fkey"
        FOREIGN KEY ("studentId") REFERENCES "student_profiles"("id") ON DELETE CASCADE ON UPDATE CASCADE;
    EXCEPTION WHEN duplicate_object THEN NULL; END $$;`,
    `CREATE INDEX IF NOT EXISTS "placement_attempts_studentId_idx" ON "placement_attempts"("studentId");`,
  ];
  ...
  return NextResponse.json({
    message: "Placement schema migration applied successfully (safe to re-run).",
    appliedStatements: applied,
    reminder:
      "Delete this route (src/app/api/admin/apply-placement-schema-migration) or remove SEED_ADMIN_SECRET from Vercel once you're done.",
  });
}
